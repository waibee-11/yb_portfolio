export default function ExperienceCard({title, description, tech, org, duration}: {title:string, description:string, tech:Array<string>, org:string, duration:string}){
    return (
        // <div>
        //     <div>
                ...
                <div className="card-body flex flex-col max-w-lg">
                    ...
                    <div>
                        <div className="flex gap-2 flex-wrap">
                            {tech?.map((key, item) => {
                                return (
                                    <div key={key} className="badge badge-primary-content"><b>{item}</b> {key}</div>
                                )
                            })}
                        </div>
                    </div>
